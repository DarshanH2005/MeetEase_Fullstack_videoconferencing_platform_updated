...
import { useSpeakerDetection, useVideoLayoutManager, useConnectionQuality } from "../../hooks/useMeetingHooks";
...
export default function VideoMeet() {
  ...
  } = useSpeakerDetection(videos.map(v => ({
    ...v,
    stream: v.stream,
  ...
  const handleParticipantClick = (participant) => {
    if (spotlightParticipant === participant.socketId) {
      clearSpotlight();
      showToast('default', 'Spotlight cleared', `Removed ${participant.title} from spotlight`);
    } else {
      setSpotlight(participant.socketId);
      showToast('success', 'Spotlight set', `${participant.title} is now in spotlight`);
    }
  };
    ...
    if (screen) {
      if (navigator.mediaDevices.getDisplayMedia) {
        navigator.mediaDevices
          ...
          .catch((e) => {
            console.log(e);
            showToast('destructive', 'Screen share failed', 'Unable to start screen sharing');
          });
      }
    }
  };
  ...
  const getPermissions = async () => {
    try {
      ...
      showToast('success', 'Permissions granted', 'Camera and microphone access enabled');
    } catch (err) {
      console.log(err);
      showToast('destructive', 'Permission denied', 'Please enable camera and microphone access');
    }
  };
  ...
  const getUserMedia = () => {
    if ((video && videoAvailable) || (audio && audioAvailable)) {
      navigator.mediaDevices
        ...
        .catch((e) => {
          console.log(e);
          showToast('destructive', 'Media access failed', 'Unable to access camera or microphone');
        });
    }
  };
  ...
  const connectToSocketServer = () => {
    ...
    socketRef.current.on("chat-message", (data, sender, socketId, timestamp) => {
      ...
      if (sender !== username) {
        ...
        if (data.length > 20) {
          showToast('default', 'New message', `${sender}: ${data.substring(0, 20)}...`);
        } else {
    ...
    socketRef.current.on("user-left", (id) => {
      ...
      showToast('warning', 'Participant left', `${participantName} has left the meeting`);
    });
    ...
    socketRef.current.on("user-joined", (id, clients, username) => {
        ...
        if (!connections[socketListId] && socketListId !== socketIdRef.current) {
          ...
          connections[socketListId].onaddstream = (event) => {
            ...
            if (!event.stream || !event.stream.getVideoTracks().length) {
              console.warn("Invalid or empty stream received from:", socketListId);
              return;
            }
            ...
            if (videoExists) {
              ...
              setVideos((videos) => {
                ...
                console.log("Added new video for:", socketListId, "Total videos:", newVideos.length);
                return newVideos;
              });
            }
          };
      ...
      if (id !== socketIdRef.current) {
        showToast('success', 'New participant', `${username || 'Someone'} joined the meeting`);
      }
    });
    ...
    socketRef.current.on("participant-joined", (data) => {
      if (data && data.participants) {
        ...
        if (data.newParticipant) {
          const { socketId, username, audioEnabled, videoEnabled, connectionQuality } = data.newParticipant;
          ...
          showToast('info', 'Participant details', 
            `${username} joined with ${audioEnabled ? 'audio on' : 'audio off'} and ${videoEnabled ? 'video on' : 'video off'}`
          );
        }
      }
    });
  ...
  const handleInvite = () => {
    ...
    if (navigator.share) {
      ...
      navigator.share({
        ...
        showToast('success', 'Invite sent', 'Meeting link shared successfully');
      }).catch((error) => {
  ...
  const fallbackCopyInvite = (meetingUrl) => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(meetingUrl).then(() => {
        showToast('success', 'Invite copied', 'Meeting link copied to clipboard');
      }).catch(() => {
    ...
    videos.forEach(video => {
      if (video.stream) {
        video.stream.getTracks().forEach(track => {
          console.log("Cleaning up track:", track.kind, "for socket:", video.socketId);
          track.stop();
        });
      }
    });
  ...
  const handleVideoLoad = useCallback((socketId) => {
  ...
  const handleVideoError = useCallback((socketId) => {
  ...
  const getAvailableDevices = async () => {
    try {
      ...
      showToast('destructive', 'Device Error', 'Could not access media devices');
    }
  };
  ...
  const changeAudioDevice = async (deviceId) => {
    try {
      ...
      showToast('success', 'Audio Device Changed', 'Microphone updated successfully');
    } catch (error) {
  ...
  const changeVideoDevice = async (deviceId) => {
    try {
      ...
      showToast('success', 'Video Device Changed', 'Camera updated successfully');
    } catch (error) {
  ...
  const testSpeaker = () => {
    ...
    setTimeout(() => {
      ...
      showToast('success', 'Speaker Test Complete', 'Did you hear the test tone?');
    }, 2000);
  };
  ...
  useEffect(() => {
    if (activeSpeaker && speakerDetectionSupported) {
      const speakerName = participants.get(activeSpeaker) || `User ${activeSpeaker.substr(0, 4)}`;
  ...
  useEffect(() => {
    qualityMetrics.forEach((metric, participantId) => {
      if (metric.status === 'poor') {
        const participantName = participants.get(participantId) || `User ${participantId.substr(0, 4)}`;
        showToast('warning', 'Poor connection', `${participantName} has connection issues`);
      }
    });
  ...
  const meetingControls = [
    ...
    },
    {
      ...
      icon: screen ? <StopScreenShareRoundedIcon /> : <PresentToAllRoundedIcon />,
  ...
  const setVideoRef = useCallback((videoElement, stream, socketId) => {
    if (videoElement && stream && videoElement.srcObject !== stream) {
      ...
      console.log("Video element stream set:", stream.id, "for socket:", socketId);
  ...
  return (
    <StyledBox>
      ...
      >
          ...
          >
            <motion.button
              ...
              style={{
                ...
                background: currentPage === 0 ? "rgba(255, 255, 255, 0.05)" : "rgba(102, 126, 234, 0.2)",
            ...
            <motion.button
              ...
              style={{
                ...
                borderRadius: "8px",
                border: "1px solid rgba(255, 255, 255, 0.2)",
                background: currentPage >= totalPages - 1 ? "rgba(255, 255, 255, 0.05)" : "rgba(102, 126, 234, 0.2)",
                color: currentPage >= totalPages - 1 ? "rgba(255, 255, 255, 0.4)" : "white",
        ...
        {askUsername === true ? (
          ...
          <>
            ...
            <motion.div
              initial={{ x: isMobile ? 0 : 400, y: isMobile ? 500 : 0, opacity: 0 }}
              animate={{ 
              ...
              >
                ...
                >
                  ...
                  <div style={{ marginBottom: "24px" }}>
                    <h3 style={{ color: "#64748b", marginBottom: "16px" }}>Device Selection</h3>
                  ...
                  <div style={{ marginBottom: "24px" }}>
                    <h3 style={{ color: "#64748b", marginBottom: "16px" }}>Test Devices</h3>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ 
                        ...
                        <span style={{ color: "white", fontWeight: "500" }}>Microphone Test</span>
                        <Button
                      ...
                      {isTestingMic && (
                        <div>
                          <p style={{ color: "rgba(255, 255, 255, 0.7)", fontSize: "0.85rem", margin: "4px 0" }}>
                          ...
                          <LinearProgress 
                            ...
                            sx={{
                              ...
                              '& .MuiLinearProgress-bar': {
                                backgroundColor: micTestLevel > 30 ? '#10b981' : '#64748b',
                              }
                            }}
                          />
                        </div>
                    ...
                    <div style={{ 
                      ...
                      <div style={{ 
                        ...
                        <span style={{ color: "white", fontWeight: "500" }}>Speaker Test</span>
                        <Button
                          onClick={testSpeaker}
                  ...
                  <div style={{ marginBottom: "20px" }}>
                    <h3 style={{ color: "#64748b", marginBottom: "12px" }}>Audio & Video</h3>
                    <div style={{ marginBottom: "16px" }}>
                      <div style={{ 
                        display: "flex", 
                        justifyContent: "space-between", 
                  ...
                  {/* Meeting Preferences */}
                  <div style={{ marginBottom: "20px" }}>
                    <h3 style={{ color: "#64748b", marginBottom: "12px" }}>Meeting Preferences</h3>
                    ...
                    <div style={{ 
                      ...
                      <span style={{ color: "white" }}>Join with muted mic</span>
                      <Switch
            ...
            >
              ...
              >
                ...
                >
                  <div style={{ marginBottom: "20px" }}>
                    <h3 style={{ color: "#f97316", marginBottom: "12px" }}>Connection Status</h3>
                    <div style={{ 
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                        <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Socket ID:</span>
                        <span style={{ color: "white", fontFamily: "monospace", fontSize: "0.8rem" }}>
                          {socketIdRef.current || 'Not connected'}
                        </span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                        <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Participants:</span>
                        <span style={{ color: "white" }}>{videos.length + 1}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Room:</span>
                        <span style={{ color: "white" }}>{state.user.meetingRoom || 'Unknown'}</span>
                      </div>
                    </div>
                  </div>

                  <div style={{ marginBottom: "20px" }}>
                    <h3 style={{ color: "#f97316", marginBottom: "12px" }}>Media Stats</h3>
                    <div style={{ fontSize: "0.9rem" }}>
                      <div style={{ 
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                          <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Video Quality:</span>
                          <span style={{ color: "#10b981" }}>HD</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                          <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Audio Bitrate:</span>
                          <span style={{ color: "white" }}>64 kbps</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Video Bitrate:</span>
                  ...
                  <div style={{ marginBottom: "20px" }}>
                    <h3 style={{ color: "#f97316", marginBottom: "12px" }}>Browser Info</h3>
                    <div style={{ fontSize: "0.9rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                        <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>User Agent:</span>
                        <span style={{ color: "white", fontSize: "0.8rem" }}>
                          {navigator.userAgent.substring(0, 30)}...
                        </span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                        <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>WebRTC Support:</span>
                        <span style={{ color: "#10b981" }}>Yes</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                        <span style={{ color: "rgba(255, 255, 255, 0.8)" }}>Screen Share:</span>
                        <span style={{ color: screenAvailable ? "#10b981" : "#ef4444" }}>
            ...
            >
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  display: "grid",
                  gridTemplateColumns: 
                    currentVideos.length === 1 ? "1fr" : 
                    currentVideos.length === 2 ? "1fr 1fr" : 
                    currentVideos.length <= 4 ? "1fr 1fr 1fr 1fr" :
                    isMobile ? "1fr 1fr" : "1fr 1fr 1fr 1fr", // Mobile: 2 columns, Desktop: 4 columns
                  gridTemplateRows: 
                    currentVideos.length === 1 ? "1fr" :
                    currentVideos.length === 2 ? "1fr" :
                    currentVideos.length <= 4 ? "1fr" :
                    isMobile ? "1fr 1fr 1fr 1fr" : "1fr 1fr", // 4x2 grid
                ...
                {currentVideos.length === 0 ? (
                    ...
                    currentVideos.map((video, index) => {
                      const participantName = participants.get(video.socketId) || video.name || `User ${video.socketId.substr(0, 4)}`;
                      const isActiveSpeaker = activeSpeaker === video.socketId;
                      const isLoading = videoLoadingStates.get(video.socketId) || false;
                      const hasError = videoErrors.get(video.socketId) || false;
                      const quality = connectionQualities.get(video.socketId) || 'good';
                      ...
                      return (
                        <motion.div
                          ...
                          style={{
                            ...
                            maxHeight: currentVideos.length === 1 ? "400px" : 
                                      isMobile ? "150px" : "200px",
                            borderRadius: "12px",
                            overflow: "hidden",
                            border: isActiveSpeaker ? "3px solid #667eea" : "2px solid rgba(255, 255, 255, 0.2)",
                          ...
                          onClick={() => handleParticipantClick(video.socketId)}
                        >
                          {/* Loading Overlay */}
                          {isLoading && (
                            <div style={{
                          ...
                          <video
                            ...
                            ref={(ref) => setVideoRef(ref, video.stream, video.socketId)}
                            autoPlay
                            playsInline
                            muted={false}
                            onLoadStart={() => setVideoLoading(video.socketId, true)}
                            onLoadedData={() => handleVideoLoad(video.socketId)}
                          ...
                          <div style={{
                            ...
                            <div style={{
                              ...
                              height: "6px",
                              borderRadius: "50%",
                              background: quality === 'good' ? "#10b981" : 
                                         quality === 'medium' ? "#f59e0b" : "#ef4444"
                            }} />
                            <span style={{
                              color: "white",
                              fontSize: "9px",
                          ...
                          <div style={{
                            ...
                            {!audioAvailable && (
                              <div style={{
                                ...
                                <MicOffIcon style={{ fontSize: "14px", color: "white" }} />
                              </div>
                            )}
                            {!videoAvailable && (
                              <div style={{
                                ...
                                <VideocamOffIcon style={{ fontSize: "14px", color: "white" }} />
                              </div>
                            )}
                          </div>
                        </motion.div>
                ...
                <div style={{
                  ...
                  {!audioAvailable && (
                    <div style={{
                      ...
                      <MicOffIcon style={{ fontSize: "12px", color: "white" }} />
                    </div>
                  )}
                  {!videoAvailable && (
                    <div style={{
                      ...
                      <VideocamOffIcon style={{ fontSize: "12px", color: "white" }} />
                    </div>
                  )}
                </div>
              </motion.div>
            ...
            >
              <motion.div 
                ...
                whileHover={{
                  boxShadow: '0 0 35px rgba(102, 126, 234, 0.6), 0 25px 50px -12px rgba(0, 0, 0, 0.6)',
                  scale: 1.005,
                  transition: { 
                    type: "spring",
                    stiffness: 300,
                ...
                style={{
                  padding: '2px',
                  borderRadius: '18px',
                  background: 'linear-gradient(45deg, rgba(102, 126, 234, 0.4), rgba(118, 75, 162, 0.4), rgba(96, 165, 250, 0.4))',
                  boxShadow: '0 0 20px rgba(102, 126, 234, 0.3), 0 25px 50px -12px rgba(0, 0, 0, 0.5)',
                }}
              >
                <div style={{ 
                  borderRadius: '16px',
      ...
      </motion.div> {/* Close main viewport container */}
    </StyledBox>
  );
};
